  ...
  const handleSubmit = async (event) => {
    ...
    if (!tipoAccion) {
      Swal.fire({
        ...
        text: 'Por favor completa el campo Tipo de Acción.',
      });
      return;
    }

    try {
      const url = `${getIpApis()}/helpers/insert/tipoaccion`;
        ...
        <div className="fixed left-0 top-0 w-full h-full bg-black bg-opacity-50 z-50 overflow-auto backdrop-blur flex justify-center items-center">
          <div className="bg-white m-auto p-8 rounded-lg shadow-lg w-full max-w-lg">
            <div className="bg-[#003865] flex items-center w-full mb-10 p-4 rounded-t-lg">
              <p className="text-2xl text-white text-left flex-grow">Nuevo Tipo de Acción</p>
              <Image 
                src={logo}
                alt="logo"
                width={50} 
              ...
              <form className="space-y-4 w-full" onSubmit={handleSubmit}>
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 flex flex-col">
                    <label className="block text-[#8B8E98] text-xs font-semibold mb-1">Tipo de Acción (*)</label>
                    <input
                      type="text"
                      value={tipoAccion}
                      placeholder="Ingresa el tipo de acción"
...
export default ModalTipoAccion;
